    ...
    for episode in range(numEpisode):
        ...
        for step in range(k):
            ...
            if(step > windowSize):
                ...
                agent.memorize(episode, step, (env.seeds[:-1]).copy(), (env.candidates + [action]).copy(), long_term_reward, mu_s, mu_c, mu_v)
...
def predict():
    util = Util()
    graph, embedding, graph_size, top_nodes_idx, node2neighbors, idx2node, node2idx, gcn_embedding = util.load_dataset("./dataset/test/", k, 0.003)
