...
def run_cora():
    ...
    # features.cuda()
    enc1 = MeanAggregator(features, 1433, 128, adj_lists, cuda=False)
    enc2 = MeanAggregator(lambda nodes : enc1(nodes).t(), 128,128, adj_lists, enc1, cuda=False)
    enc1.num_samples = 5
    enc2.num_samples = 5
    '''
    ...
    sage1 = GraphSAGE(features, num_nodes, layer_infos, 0, adj_lists)
    sage2 = GraphSAGE(lambda x:sage1(x).t(), num_nodes, layer_infos,1, adj_lists)
    # sage = GraphSAGE(feat_data, num_nodes, feat_dim, layer_infos, adj_lists)
    graphsage = SupervisedGraphSage(7, sage2)
...
def run_pubmed():
   ...
   # features.cuda()
    ...
    agg1 = MeanAggregator(features, cuda=True)
    enc1 = Encoder(features, 500, 128, adj_lists, agg1, gcn=False, cuda=False)
    agg2 = MeanAggregator(lambda nodes : enc1(nodes).t(), cuda=False)
    enc2 = Encoder(lambda nodes : enc1(nodes).t(), enc1.embed_dim, 128, adj_lists, agg2,
            base_model=enc1, gcn=False, cuda=False)
    enc1.num_samples = 10
